...
int work::auth_passwd(int userid, const char *passwd)//验证用户密码
{
	...
	if (strncmp(passwd, "123456", 6) == 0)//如果passwd为"123456"，代表验证通过
		return 1;
	return 0;
}
...
void work::run()
{
	struct epoll_event ev, events[CLIENTCOUNT + 1]; //声明epoll_event结构体的变量,ev用于注册事件,数组用于回传要处理的事件
	memset(&ev, 0, sizeof(ev));
	memset(events, 0, sizeof(events));

	setnonblocking (listen_st); //把socket设置为非阻塞方式
	int epfd = epoll_create(CLIENTCOUNT + 1); //建立epoll描述符
	...
	int st = 0;//定义一个socket，代表来自client端的socket连接
	while (1)
	{
		int nfds = epoll_wait(epfd, events, CLIENTCOUNT + 1, -1); //阻塞，等待epoll事件的发生
		if (nfds == -1)
		{
		...
		for (int i = 0; i < nfds; i++)
		{
			...
			{
				st = socket_accept(listen_st);//将来自client端的socket描述符设置为非阻塞
				if (st >= 0)
				{
					setnonblocking(st);
					ev.data.fd = st;
					ev.events = EPOLLIN | EPOLLERR | EPOLLHUP; //设置要处理的事件类型	
					if (epoll_ctl(epfd, EPOLL_CTL_ADD, st, &ev) == -1)//将来自client端的socket描述符加入epoll
					{
						printf("epoll_ctl error , %s\n", strerror(errno));
					}
					continue;
			...
			if (events[i].events & EPOLLIN) //socket收到数据
			{
				...
				if (socket_recv(st) <= 0)
				{
					if (epoll_ctl(epfd, EPOLL_CTL_DEL, st, NULL) == -1)//如果socket出错，将socket从epoll移除
					{
						printf("epoll_ctl error , %s\n", strerror(errno));
					}
					user_logout(st);
