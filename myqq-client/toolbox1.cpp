...
bool toolbox1::eventFilter(QObject *watched, QEvent *event)
{
    ...
    //由于重写了父类的同名函数，所以需要调用父类eventFilter函数进行默认消息处理
...
{
    qDebug() << passwd;
    qDebug() << passwd.toStdString().c_str();
    qDebug() << passwd.toLatin1().data();

    QByteArray bt = passwd.toLatin1();
    const char *pw = bt.data();
    qDebug() << pw;
